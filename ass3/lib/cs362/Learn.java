...
Learn {
	...
	private static Predictor train(List<Instance> instances, String algorithm) {
		...
		if(algorithm.equalsIgnoreCase("kernel_logistic_regression")){
			KernelLogisticRegression klr = new KernelLogisticRegression(instances,kernel,polynomial_kernel_exponent,
					gaussian_kernel_sigma, gradient_ascent_learning_rate,gradient_ascent_training_iterations);
			klr.train(instances);
			return klr;
		}
